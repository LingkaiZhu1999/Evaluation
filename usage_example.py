...
parser.add_argument("-rp", "--read_path", type=str, default="D:/Split_Validation_calPSNR2/res", help="Path to the training dataset")
parser.add_argument("-wp", "--write_path", type=str, default="D:/Split_Validation_calPSNR2/res", help="Path to write output images")
...
image_medium = cv2.cvtColor(cv2.imread(read_path + "{:04d}.png".format(image_id), cv2.IMREAD_UNCHANGED), cv2.COLOR_BGR2RGB) / 255.0
...
image_medium_corrected = (((image_medium**gamma)*2.0**(-1*floating_exposures[1]))**(1/gamma))
# Do some processing to the input image(s) to obtain an output image
output_image = (image_short_corrected+image_medium_corrected+image_long_corrected)/3
